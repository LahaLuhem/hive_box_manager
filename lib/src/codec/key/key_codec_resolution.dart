import 'int_key_codec.dart';
import 'key_codec.dart';
import 'string_key_codec.dart';

/// Resolves the [KeyCodec] a keyed façade wires for [K]: an [explicitCodec] always wins, and `int` /
/// `String` keys default to the identity codecs.
///
/// Any other [K] without a codec trips an assert while wiring. Construction always runs in development
/// and the check doesn't depend on data, so the assert is the real contract. The [ArgumentError] behind
/// it is the release fallback, since a codec-less box can't work at all.
KeyCodec<K> resolveKeyCodec<K extends Object>(KeyCodec<K>? explicitCodec) {
  if (explicitCodec != null) return explicitCodec;

  assert(
    K == int || K == String,
    'No KeyCodec<$K> given: only int and String keys default to identity codecs. Pass codec:.',
  );

  // Exact type equality first. A bare `is` would let a supertype like `Object` slip into an identity
  // codec and blow up at the first put instead of here.
  if (K == int || K == String) {
    final identityCodec = K == int ? const IntKeyCodec() : const StringKeyCodec();
    if (identityCodec is KeyCodec<K>) return identityCodec;
  }

  throw ArgumentError.value(K, 'codec', 'no KeyCodec given, and $K has no identity default');
}
