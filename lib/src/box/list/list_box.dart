...
interface class ListBox<T extends Object, K extends Object> {
  ...
  /// The stored lists, each an unmodifiable view, decoded as they are iterated. Dispatches one
  /// read-all event at call time.
  Iterable<List<T>> get values => _engine.values;

  /// Reads the list under [key] synchronously from memory: `Some` of an unmodifiable zero-copy
  /// view when present (`Some(empty)` for a stored empty list), `None` when the key is absent.
  Option<List<T>> get(K key) => _engine.get(key);
  ...
  /// (multimap-natural). Sugar over [update]: a read-modify-write, O(n) in the stored list.
  ...
  /// a copy of [values]. Sugar over [update]: a read-modify-write, O(n) in the stored list.
  ...
  /// read-modify-write, O(n) in the stored list.
  Task<Unit> remove(K key, T value) => Task(() async {
    final storedValues = _engine.get(key).toNullable();
  ...
  /// Typed change stream; pass [key] to watch one key only. Event payloads carry the same
  /// unmodifiable views reads do, non-null even on deletes (the eager promise).
  Stream<TypedBoxEvent<List<T>, K>> watch({K? key}) => _engine.watch(key: key);
  ...
  /// `Hive.initFlutter()`) plus adapter registration for the **element** type [T]. [codec]
  /// defaults by key type (`int` / `String` identity codecs; any other [K] without an explicit
