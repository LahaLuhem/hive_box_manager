...
final class DualIntIndexBoxManager<T> extends _BaseDualIndexBoxManager<T, int, int, int> {
  ...
  /// ### Collision Test
  /// Empirically tested up to 20,000 possible indices
  factory DualIntIndexBoxManager.bitShift({required String boxKey, required T defaultValue}) =>
      DualIntIndexBoxManager._(
        boxKey: boxKey,
        defaultValue: defaultValue,
        encoder: bitShiftEncoder,
      );
  ...
  /// ### ❌ Cons:
  /// + Limited range - Only handles ±16,383
  factory DualIntIndexBoxManager.negative({required String boxKey, required T defaultValue}) =>
      DualIntIndexBoxManager._(
        boxKey: boxKey,
        defaultValue: defaultValue,
        encoder: negativeNumbersEncoder,
...
@protected
@visibleForTesting
int bitShiftEncoder(int primaryIndex, int secondaryIndex) {
  assert(
