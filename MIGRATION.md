...
`0.0.x` to any current `1.x` release is a from-scratch rewrite: the manager classes are gone, and
the new box façades carry a different contract. The good news sits in the first table below: your
data almost always reads in place.
...
(range ±16383 per part). 1.x doesn't reship it: the default `StringCompositeDualCodec` covers
